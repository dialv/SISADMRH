...
public class PersonalCapacitadoView extends AbstractXlsView{
@Override
...
// response.setHeader("Content-Disposition", "<span id="IL_AD8" class="IL_AD">attachment</span>;filename=\"student.xls\"");    
 ...
 List<Object[]> estadisticocapacitadoList = (List<Object[]>) model.get("estadisticocapacitadoList");
 Sheet sheet = workbook.createSheet("Data Personal Capacitado");
 sheet.createRow(0).createCell(3).setCellValue("");
  sheet.createRow(1).createCell(4).setCellValue("REPORTE DE PERSONAL CAPACITADO");
 Row header = sheet.createRow(2);

 header.createCell(2).setCellValue("No Empleado");
 header.createCell(3).setCellValue("Ubicacion");
 header.createCell(4).setCellValue("No Capacitaciones");
 header.createCell(5).setCellValue("Temas de Capacitacion");
 header.createCell(6).setCellValue("Fecha de Capacitacion");
  
 int rowNum = 3;
 for(Object[] List : estadisticocapacitadoList){
